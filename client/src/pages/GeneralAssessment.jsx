...
const GeneralAssessment = () => {
  ...
  const validate = () => {
    ...
    if (!formData.usingDrugs) newErrors.usingDrugs = 'This field is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification('General assessment saved successfully', 'success');
      navigate('/patients');
    } catch {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="card">
        <div className="page-header bg-gradient-to-r from-emerald-500 to-teal-500">
          <div className="flex items-center gap-3">
            <CheckCircle className="w-8 h-8" />
            <div>
              <h2 className="text-2xl font-bold">General Assessment</h2>
              <p className="text-emerald-100 text-sm mt-1">BMI indicates normal weight (BMI ≤ 25)</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <RadioGroup
              ...
              name="usingDrugs"
              options={['Yes', 'No']}
              value={formData.usingDrugs}
              onChange={(val) => setFormData(prev => ({ ...prev, usingDrugs: val }))}
              error={errors.usingDrugs}
            />
          </div>
